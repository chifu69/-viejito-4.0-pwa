  ...
  try{
    if(intent==='bw'){
      if(vals.length>=2){
        ...
        return {kind:'result',title:'Basis Weight',value:fmt(result),meta:mandrel===48?t('defaultMandrel',{m:mandrel}):t('mandrelOnly',{m:mandrel})};
      }
      const n=vals[0];
    ...
    if(intent==='ft'){
      if(vals.length>=2){
        ...
        return {kind:'result',title:'Feet',value:`${fmt(result,0)} ft`,meta:mandrel===48?t('defaultMandrel',{m:mandrel}):t('mandrelOnly',{m:mandrel})};
      }
      const n=vals[0];
    ...
    if(intent==='swrap'){
      if(vals.length>=3){
        ...
        return {kind:'result',title:'S-Wrap Speed',value:fmt(result,1),meta:t('newRecommendedSpeed')};
      }
      const n=vals[0];
...
function recalculateWithMandrel(mandrel){
  ...
  try{
    if(c.intent==='bw'&&positive(c.weight,c.length)){
      ...
      return {kind:'result',title:'Basis Weight',value:fmt(result),meta:t('recalculatedMandrel',{m:mandrel})};
    }
    if(c.intent==='ft'&&positive(c.bw,c.weight)){
      ...
      return {kind:'result',title:'Feet',value:`${fmt(result,0)} ft`,meta:t('recalculatedMandrel',{m:mandrel})};
    }
